...
mydb=mysql.connector.connect(host='localhost',user='nandini',passwd='P@ssword_20')#password depending on the user pc
mc=mydb.cursor()
mc.execute("use dbms")
...
def Hindi():
    try:
        ...
        if final=="sad":
            ...
            y=len(x)
            webbrowser.open(x[2:y-3]) #https-----.com.
        if final=="angry" or final=="jealousy":
            mc.execute("use dbms")
            mc.execute("select link from Aplaylist where lang='hindi';")
            op=mc.fetchall() #op= type "list", and the elements are of type "tuple":[('https://',),('https:',)]
            res=random.randint(0,len(op)) #ex: res=3
            myLink.set(op[res]) #myText= op[3]
            x=str(op[res]) # "('https://youtube.com/playlist?list=PLB00151086C9310A0',)"
        ...
        if final=="excited":
            mc.execute("use dbms")
            mc.execute("select link from Eplaylist where lang='hindi';")
            op=mc.fetchall() #op= type "list", and the elements are of type "tuple":[('https://',),('https:',)]
            res=random.randint(0,len(op)) #ex: res=3
            myLink.set(op[res]) #myText= op[3]
            x=str(op[res]) # "('https://youtube.com/playlist?list=PLB00151086C9310A0',)"
...
def English():
    try:
        ...
        if final=="angry" or final=="jealousy":
            mc.execute("use dbms")
            mc.execute("select link from Aplaylist where lang='english';")
            op2=mc.fetchall()
            res2=random.randint(0,len(op2))
            myLink.set(op2[res2])
            z=str(op2[res2])
        ...
        if final=="excited":
            mc.execute("use dbms")
            mc.execute("select link from Eplaylist where lang='english';")
            op2=mc.fetchall()
            res2=random.randint(0,len(op2))
            myLink.set(op2[res2])
            z=str(op2[res2])
...
def emotion():
    ...
    ts = 0
    found = None
    for file_name in glob.glob("D:\music_recom\MusicRecommendationSystem-main\EMOTICA\*"): #subject to your path where musicgen.py is saved
...
root.iconbitmap("D:\music_recom\MusicRecommendationSystem-main\EMOTICA\img.ico") #subject to your path where the img.ico is saved -> in the same folder as musicgen.py
